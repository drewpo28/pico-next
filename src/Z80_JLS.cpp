    ...
    if (ESPectrum::trdos == true || Z80Ops::is128) {
        ...
        if (!ESPectrum::trdos) {
            ...
            if (REG_PCh == 0x3D) {
                ...
                // TR-DOS ROM is accessible from 128K when the currently mapped
                // ROM is bank 1 (48K Basic). The 0x3Dxx trap then swaps in the
                // TR-DOS ROM.
        ...
        if ((Tape::tapeFileType == TAPE_FTYPE_TAP || Tape::tapeFileType == TAPE_FTYPE_TZX || Tape::tapeFileType == TAPE_FTYPE_PZX) && (Tape::tapeFileName != "none")) {
              ...
              if (Config::flashload && !Tape::jjScreenAnimating) {
                ...
                if (Tape::FlashLoad()) {
                    ...
                    if (trapPC == 0x57d) REG_PC = 0x606; // Sinclair ROM
