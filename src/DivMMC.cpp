...
void DivMMC::init() {
    ...
    if (!enabled) {
        ...
        // contiguous blocks after a toggle cycle fails on tight-heap boards
        // (ZERO2 without PSRAM, ~17 KB free) due to fragmentation. Keep the
        // slots and the swap file open across toggles — they are reused on
        // next enable. Slot bookkeeping is reset so the cache acts empty.
        if (!use_psram) {
